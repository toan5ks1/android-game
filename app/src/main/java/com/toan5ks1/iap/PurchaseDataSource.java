...
import com.toan5ks1.iap.SampleIapManager.PurchaseRecord;
...
public class PurchaseDataSource {
    ...
    private PurchaseRecord cursorToPurchaseRecord(final Cursor cursor) {
        ...
        purchaseRecord.setReceiptId(cursor.getString(cursor.getColumnIndexOrThrow(SampleSQLiteHelper.COLUMN_RECEIPT_ID)));
        purchaseRecord.setUserId(cursor.getString(cursor.getColumnIndexOrThrow(SampleSQLiteHelper.COLUMN_USER_ID)));
        try {
            purchaseRecord.setStatus(PurchaseStatus.valueOf(cursor.getString(cursor.getColumnIndexOrThrow(SampleSQLiteHelper.COLUMN_STATUS))));
     ...
     * @return purchase record associated with the receipt Id and userId
     */
